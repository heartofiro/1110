...
public class Player
{
    ...
    public enum State { ROTATION, MOVEMENT, PLACEMENT, IDLE}
    ...
    public Player(String playerString)
        ...
        String moneyString = playerString.substring(2, 5);//Get string containing 3rd to 5th char which representing number of play's money
        this.playerMoney = Integer.parseInt(moneyString); //Transfer moneyString to int

        // Obtain the player's rugs left
        String rugsString = playerString.substring(5, 7); //Get string containing 6th to 7th char which representing number of play's rug
        this.rugsLeft = Integer.parseInt(rugsString);     //Transfer rugsString to int

        // Obtain whether the player is in or out
        playerState = playerString.charAt(7);         //Get 7th char in playerString
        if (playerState == 'i'){                           //If satusChar is 'i',  representing player is in game
            this.inGame = true;
        } else if (playerState == 'o') {                   //If satusChar is 'i',  representing player is out
            this.inGame = false;
    ...
    public void transferMoney(Player receiver, int amount) //If player'money more than they need to pay the receiver, the amount of money would transfer to receiver.
    ...
    /**
     * Override method for displaying a Player as a string
    ...
    {
        String inGameStr;
        if(this.inGame) inGameStr = "IN"; else inGameStr = "OUT";
        return String.format("%s Player\n%s Dirhams\n%s\nRemaining Rugs: %s", this.playerColour, this.playerMoney, inGameStr, this.rugsLeft);
    }
